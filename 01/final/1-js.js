...
function onClick() {
    alert(
        `Hello you have clicked on the anchor ${n} times ${Intl.DateTimeFormat().format()}`
    )
    n = n + 1
}
...
newElement.innerHTML = Intl.DateTimeFormat(
    'it-IT',
    longMonthDateFormat
).format()

// Solution without using a variable
const tdElement = document.getElementsByTagName('td')
console.log(2, tdElement)

for (let i = 0; i < tdElement.length; i++) {
    console.log(3, tdElement[i])
    tdElement[i].addEventListener('click', function () {
        alert('Hello')
    })
}

// Or you can convert the result of getElementsByTagName to an array
const tdElements = Array.from(tdElement)
